...
`npm run build` 输出 `dist/`，只包含网站运行文件，不含本地工具、测试截图、原始下载地址或大体积原片。当前 182 文件、498.2 MiB，均低于 GitHub Pages 容量限制，每个文件低于 GitHub 100 MiB 限制。资源路径全部为相对路径，支持项目子目录部署。
...
对全片每秒抽样比较（62 帧），SSIM 均值 0.99484、最低 0.98485。其他三个视频沿用现有本地高清文件，录屏黑边通过已有容器裁切参数处理，画面不拉伸。
